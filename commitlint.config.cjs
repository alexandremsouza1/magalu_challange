module.exports = {
	...
	prompt: {
		messages: {
			...
			footer:
				"Liste os issues fechados por esta mudança (opcional). Ex: #31, #34:\n",
		...
		questions: {
			type: {
				...
				enum: {
					...
					refactor: {
						description:
							"Uma mudança de código que não corrige bug nem adiciona funcionalidade",
					...
					test: {
						description:
							"Adição de testes faltando ou correção de testes existentes",
						title: "Tests",
						emoji: "🚨",
					},
					build: {
					...
					ci: {
						description: "Mudanças em arquivos e scripts de configuração de CI",
			...
			scope: {
				description:
					"Qual é o escopo desta mudança (frontend, backend, api, etc)",
			},
		},
	},
};
